    ...
    def affichage(self,grille,listebob):
        ...
        for y in range(TAILLE):
            for x in range(TAILLE):
                ...
                # if grille[x][y].food == 0:
                #     self.fenetre.blit(self.sol, (150 + 16 * (y - 1) + 16 * x, 8 * (y - 1) + 520 - 8 * x))
                # else:
                #     self.fenetre.blit(self.food, (150 + 16 * (y - 1) + 16 * x, 8 * (y - 1) + 520 - 8 * x))
                if not(grille[x][y].food ==0):
                    self.fenetre.blit(self.food, (935 + x * 21 - 21 * y,3 + y * 12 + x * 12))
            # self.fenetre.blit(self.perso, (939 + 0 * 21 - 21 * 0, 8 + 0 * 12 + 0 * 12))
        # Affichage des Bobs
        for bob in listebob:
            x, y = bob.i, bob.j
